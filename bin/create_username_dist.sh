...
cd "$DIR" || exit
cat failed_login_data.txt| sort | awk '{print $4}' |\
         uniq -c | awk '{print "data.addRow([\x27"$2"\x27,"$1"]);" }' > tempFile.txt

#Need to hand tempFile.txt to wrap_contents.sh
cd "$curDir" || exit

./bin/wrap_contents.sh "$DIR"/tempFile.txt /html_components/username_dist "$DIR"/username_dist.html
rm "$DIR"/tempFile.txt
